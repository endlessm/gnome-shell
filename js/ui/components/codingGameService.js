...
var CodingChatboxTextService = new Lang.Class({
    ...
    _init: function() {
        ...
        this._eventHandlers = {
            'move-window': this._waitForNextWindowMove.bind(this),
            'stop-moving-windows': this._waitForNoWindowsToMove.bind(this)
        };
    },
    ...
    enable: function() {
        ...
        this._controller.service_event_with_listener('move-window', () => {
            this._startListeningForEvent('move-window');
        }, () => {
            this._disconnectHandlersFor('move-window');
        });
        this._controller.service_event_with_listener('stop-moving-windows', () => {
    ...
    disable: function() {
        Object.keys(this._signalHandlers, (key) => {
    ...
    _waitForNextWindowMove: function() {
        let wmCallback = ((display, screen, window, op) => {
    ...
    _waitForNoWindowsToMove: function() {
        ...
        let grabOpBeginCb = ((display, screen, window, op) => {
            if (window && op === Meta.GrabOp.MOVING) {
        ...
        let grabOpEndCb = ((display, screen, window, op) => {
            if (window && op === Meta.GrabOp.MOVING) {
