...
var CodingSession = GObject.registerClass({
    ...
    _init(params) {
        ...
        this._hackable = true;
        this._hackableApp = new HackableApp(this);
    ...
    _synchronizeButton(window) {
        this._button.rect = window.get_frame_rect();
    }

    _synchronizeWindows(window) {
        if (!this._isCurrentWindow(window))
            return;
    ...
    _rotateInToMidpointCompleted(oldDst, newDst, direction) {
        if (oldDst) {
