...
const ForceAppExitDialogItem = GObject.registerClass({
    ...
    _init(app) {
        super._init({ style_class: 'force-app-exit-dialog-item',
                      can_focus: true,
                      reactive: true,
                      track_hover: true });
        this.app = app;
...
var ForceAppExitDialog = class extends ModalDialog.ModalDialog {
    constructor() {
        ...
        if (appSystem.lookup_app(GNOME_SYSTEM_MONITOR_DESKTOP_ID))
            this.addButton({ action: this._launchSystemMonitor.bind(this),
                             label: _("System Monitor") },
                           { x_align: St.Align.END });

        this._quitButton = this.addButton({ action: this._quitApp.bind(this),
